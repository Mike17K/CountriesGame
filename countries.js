...
function loadData(countryName3){
    fetch(`https://restcountries.com/v3/alpha/${countryName3}`).then(data=>data.json()).then((data) =>{ 
            ...
            let output = [];
            let indexesOfNaibour = [];
            naibourCountries.forEach(e => {
            // find the index of the object in the data
            for(let i=0;i<countryObjects.length;i++){
                if (countryObjects[i].code3===e){
                    indexesOfNaibour.push(i);
                    let obj={};
                    obj.src = `https://flagcdn.com/${countryObjects[i].code.toLowerCase()}.svg`;
                    obj.index = i;
                    obj.name = countryObjects[i].name
                    obj.isNai = true;
                    output.push(obj);
                };        
                }
            ...
            while(12 - output.length > 0){
                ...
                if (!isInArray){
                    let obj={};
                    obj.src = `https://flagcdn.com/${countryObjects[index_new].code.toLowerCase()}.svg`;
                    obj.index = index_new;
                    obj.name = countryObjects[index_new].name
                    obj.isNai = false;
                    output.push(obj);
                }
        ...
        output.forEach(e=>{
            ...
            console.log("Error API no output!");
        }
    }
)
}
